...
read service
echo -e "Please specify start timestamp in YYYYMMDD hh:mm:ss format or leave empty to check all file"
read vardate
startdate=$(date -d "$vardate" +%s)
rm errorlog.log 2> /dev/null
if [ -z "$service" ]
then 
	grep -i error all.log | cut -d ' ' -f 1 | cut -d '.' -f 1 > time.txt
	...
	grep -i error servicefile.txt | cut -d ' ' -f 1 | cut -d '.' -f 1 > time.txt
